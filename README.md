# FactorMiner — 强化学习 + 遗传规划 A股量化因子挖掘系统

## 项目简介

FactorMiner 是一套面向量化研究的因子挖掘系统，核心目标是在海量候选表达式空间中寻找具有稳健截面预测能力的阿尔法因子。系统综合采用**遗传规划（Genetic Programming）**和**强化学习（PPO）**两套引擎并行搜索，共享同一套算子库和评估体系，互相补充。
